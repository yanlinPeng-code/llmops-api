...
class TestBuiltinToolHandler:
    ...
    def test_get_categories(self, client):
        """测试获取所有分类信息"""
        resp = client.get("/builtin_tools/categories")
    ...
    def test_get_builtin_tools(self, client):
        """测试获取所有内置工具"""
        resp = client.get("/builtin_tools/get_all")
        assert resp.status_code == 200
        assert resp.json.get("code") == HttpCode.SUCCESS
        assert len(resp.json.get("data")) > 0
    ...
    def test_get_provider_tool(self, provider_name, tool_name, client):
        """测试获取指定工具信息接口"""
        resp = client.get(f"/builtin_tools/{provider_name}/tools/{tool_name}")
        assert resp.status_code == 200
    ...
    def test_get_provider_icon(self, provider_name, client):
        """测试根据提供商名字获取icon接口"""
        resp = client.get(f"/builtin_tools/{provider_name}/icon")
        assert resp.status_code == 200
